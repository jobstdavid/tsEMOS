...
#'
#' Additive Deseasonalized Autoregressive smooth EMOS with Generalized Autoregressive
...
#' @references
#'
#' Jobst, D., Möller, A., and Groß, J. (2024). Time Series based Ensemble Model Output Statistics for Temperature Forecasts Postprocessing.
#'
#' @examples
...
#'
#' @export
dargarchadd_semos <- function(train,
                              test,
                              doy_col,
                              obs_col,
                              mean_col,
  ...
  # combine both data frames by doy
  ...
  # fit GARCH(1,1) model
  fit_garch <- ugarchfit(spec, data = r_arm, solver = "hybrid")
  ...
  if (p != 0) {
    ...
    # get initial values for sig_c and z for the last iteration in the optimization
    MU <- doys %*% pars[1:5] + doys %*% pars[6:10]*m
    SIGMA <- as.vector(exp(doys %*% pars[11:15] + doys %*% pars[16:20]*s))
    r <- as.vector((obs-MU))
